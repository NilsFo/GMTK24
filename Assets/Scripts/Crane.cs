...
public class Crane : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (craneState is CraneState.IDLE or CraneState.MOVING)
        {
            ...
            targetPos.y = yLevel;
            var speedMod = speedCurve.Evaluate(Vector3.Distance(transform.position, targetPos) / speedCurveWidth) *
                           speed;
        ...
        else if (craneState is CraneState.GRABBING)
        {
            ...
            var speedMod = speedCurve.Evaluate(Vector3.Distance(transform.position, targetPos) / speedCurveWidth / 2f) *
                           speed;
        ...
        else if (craneState is CraneState.ROTATING)
        {
            ...
            {
                harken.rotation =
                    Quaternion.RotateTowards(harken.rotation, grabTargetRotation, rotationSpeed * Time.deltaTime);
            }
        }

        // Visuals
        laufkatze.transform.position =
            new Vector3(transform.position.x, laufkatze.transform.position.y, transform.position.z);
        ...
        seil.transform.localScale = new Vector3(seil.transform.localScale.x,
            Vector3.Distance(transform.position, laufkatze.position), seil.transform.localScale.z);
    }
    ...
    public void GrabComplete()
    {
        ...
        if (tetromino != null)
        {
            ...
            if (tetromino != null)
            {
                ...
                grabbedTile.transform.parent = transform;
                grabbedTile.transform.localPosition =
                    new Vector3(0, 0, 0) -
                    tetromino.GetAnchorPoint().transform.localPosition; // TODO correct for anchor pos & animate
    ...
    private void HandleInput()
    {
        ...
        bool leftInputRequested = false;
        bool rightInputRequested = false;
        bool upInputRequested = false;
        bool downInputRequested = false;

        bool rotateInputRequested = false;
        bool pickUpDropInputRequested = false;

        if (!useGamepadOverKBM && k != null)
        {
            leftInputRequested = k.leftArrowKey.wasPressedThisFrame;
            rightInputRequested = k.rightArrowKey.wasPressedThisFrame;
            upInputRequested = k.upArrowKey.wasPressedThisFrame;
            downInputRequested = k.downArrowKey.wasPressedThisFrame;

            rotateInputRequested = k.rKey.wasPressedThisFrame;
            pickUpDropInputRequested = k.eKey.wasPressedThisFrame;
        }

        if (useGamepadOverKBM && g != null)
        {
            leftInputRequested = g.dpad.left.wasPressedThisFrame;
            rightInputRequested = g.dpad.right.wasPressedThisFrame;
            upInputRequested = g.dpad.up.wasPressedThisFrame;
            downInputRequested = g.dpad.down.wasPressedThisFrame;

            rotateInputRequested = g.rightShoulder.wasPressedThisFrame || g.leftShoulder.wasPressedThisFrame;
            pickUpDropInputRequested = g.rightTrigger.wasPressedThisFrame || g.leftTrigger.wasPressedThisFrame;
        }
        ...
        if (craneState is CraneState.IDLE or CraneState.MOVING)
        {
            ...
            {
                if (leftInputRequested)
                {
                    SetTargetPos(gridPos.x - 1, gridPos.y);
                }
                else if (rightInputRequested)
                {
                    SetTargetPos(gridPos.x + 1, gridPos.y);
                }
                else if (upInputRequested)
                {
                    SetTargetPos(gridPos.x, gridPos.y + 1);
                }
                else if (downInputRequested)
                {
                    SetTargetPos(gridPos.x, gridPos.y - 1);
                }
            }
